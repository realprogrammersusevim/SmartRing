...
struct VilyaApp: App {
    ...
    let backgroundTaskIdentifier = "com.jonathan.vilya.logHealthData"
    ...
    var body: some Scene {
        ...
        .onChange(of: scenePhase) { _, newPhase in
            ...
            if newPhase == .active {
                ...
                VilyaApp.stopForegroundLogTimer() // Stop any existing timer
                VilyaApp.startForegroundLogTimer(
                ...
                VilyaApp.stopForegroundLogTimer()
            } else if newPhase == .background {
                print("App moved to background, scheduling health data log task.")
                VilyaApp.stopForegroundLogTimer() // Stop foreground timer
    ...
    private static func performHealthDataLoggingOperation(
        ...
        print(
            "\(operationIdentifier): Performing health data logging operation for address: \(deviceAddress)"
        )
    ...
    static func startForegroundLogTimer(
        healthDataLogger: HealthDataLogger, modelContainer: ModelContainer
    ) {
        ...
        let logAction = {
            Task {
                ...
                do {
                    try await Self.performHealthDataLoggingOperation(
                        healthDataLogger: healthDataLogger, modelContainer: modelContainer,
                        deviceAddress: deviceAddress, operationIdentifier: "ForegroundTimer"
                    )
                } catch {
        ...
        appForegroundLogTimer = Timer.scheduledTimer(withTimeInterval: BG_INTERVAL, repeats: true) {
            _ in
            logAction()
        }
        print(
            "Foreground health data log timer started. Will fire every \(BG_INTERVAL) seconds after an initial log."
        )
    }
    ...
    func handleAppRefresh(task: BGAppRefreshTask) {
        ...
        let modelConfiguration = ModelConfiguration(schema: schema, isStoredInMemoryOnly: false)
        guard
            let backgroundModelContainer = try? ModelContainer(
        ...
        Task {
            do {
                try await VilyaApp.performHealthDataLoggingOperation(
                    healthDataLogger: currentHealthDataLogger,
                    modelContainer: backgroundModelContainer,
                    deviceAddress: deviceAddressForBackgroundTask,
                    operationIdentifier: "BackgroundTask"
                )
                print("BackgroundTask (\(backgroundTaskIdentifier)): Work finished successfully.")
                task.setTaskCompleted(success: true)
            } catch {
                print(
                    "BackgroundTask (\(backgroundTaskIdentifier)): Failed to log data: \(error.localizedDescription)"
                )
                task.setTaskCompleted(success: false)
            }
        }
